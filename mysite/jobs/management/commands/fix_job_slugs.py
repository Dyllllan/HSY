"""
修复 JobPage 中 slug 为 None 的数据
使用方法: python manage.py fix_job_slugs
"""
...
class Command(BaseCommand):
    help = '修复 JobPage 中 slug 为 None 的数据'
    ...
    def handle(self, *args, **options):
        ...
        # 查找所有 slug 为 None 或空的 JobPage（使用 .specific() 确保加载具体类型）
        jobs_without_slug = JobPage.objects.live().specific().filter(slug__isnull=True) | JobPage.objects.live().specific().filter(slug='')
        
        count = jobs_without_slug.count()
        
        if count == 0:
            self.stdout.write(self.style.SUCCESS('[OK] 没有找到需要修复的数据'))
            return
        
        self.stdout.write(f'找到 {count} 个需要修复的职位页面')
        ...
        for job in jobs_without_slug:
            ...
            if not dry_run:
                try:
                    ...
                    fixed_count += 1
                    self.stdout.write(f'[OK] 修复: {job.title} -> slug: {slug}')
                ...
                self.stdout.write(f'  将修复: {job.title} -> slug: {slug}')
